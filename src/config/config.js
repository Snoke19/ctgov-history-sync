...
export const CONCURRENCY = Number(process.env.CONCURRENCY);

// Studies per list-API page. Server max is 1000.
export const PAGE_SIZE = Number(process.env.PAGE_SIZE);

// Per-request timeout in ms.
export const FETCH_TIMEOUT_MS = Number(process.env.FETCH_TIMEOUT_MS);

// Fallback wait when a 429 arrives without a Retry-After header.
export const DEFAULT_RETRY_AFTER_MS = Number(process.env.DEFAULT_RETRY_AFTER_MS);

// Base delay for exponential backoff (ms).
export const RETRY_BASE_DELAY_MS = Number(process.env.RETRY_BASE_DELAY_MS);
